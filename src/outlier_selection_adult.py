...
feature_cols = df.columns[1:]  # 忽略第一列（如果第一列是索引列）
...
# output outliers
outliers.to_csv('outcome/outlier_selection/outliers_adult.csv', index=False)

# Filter out outliers
filtered_df = df[mahal_dist <= threshold]

# save the data after filtering
filtered_df.to_csv('outcome/outlier_selection/filtered_adult.csv', index=False)
...
plt.show()
plt.savefig('outcome/outlier_selection/output_adult_fig.png', dpi=300)